...
# Fungsi untuk halaman Atur Schedule
def schedule_page():
    st.title("Atur Reminder")

    # Input tanggal dan waktu
    reminder_date = st.date_input("Pilih Tanggal", datetime.today())
    reminder_time = st.time_input("Pilih Waktu", datetime.now().time())
    description = st.text_area("Deskripsi Aktivitas")

    if st.button("Simpan Reminder"):
        ...
        st.success("Reminder berhasil disimpan!")
    ...
    if 'reminders' in st.session_state and st.session_state['reminders']:
        ...
        # Membuat kalender menggunakan plotly
        fig = make_subplots(rows=1, cols=1, specs=[[{"type": "scatter"}]])

        for date in df['date'].unique():
            daily_reminders = df[df['date'] == date]
            for idx, reminder in daily_reminders.iterrows():
                fig.add_trace(
                    go.Scatter(
                        x=[reminder['datetime']],
                        y=[1],  # Dummy y values
    ...
    if st.session_state['logged_in']:
        with st.sidebar:
            selected = option_menu(
                menu_title="Menu",
                options=["Home", "Articles", "Workout Video", "Check Your Condition", "Atur schedule", "Help", "Admin Page", "Logout"]
                if st.session_state['role'] == 'admin'
                else ["Home", "Articles", "Workout Video", "Check Your Condition", "Atur schedule", "Help", "Logout"],
                icons=["house", "book", "calculator", "check-circle", "calendar","question-circle", "gear", "door-open"],
                menu_icon="cast",
                default_index=0,
            )
